...
| Agent adapters | Built-in `process`, `http`, local CLI/session adapters, Clawith Bridge PoC, and OpenClaw gateway support; external adapters can also be loaded through the adapter plugin flow |
...
- `adapter_type` text; built-ins include `process`, `http`, `acpx_local`, `clawith_bridge`, `claude_local`, `codex_local`, `gemini_local`, `opencode_local`, `pi_local`, `cursor`, `cursor_cloud`, and `openclaw_gateway`
