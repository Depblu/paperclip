...
async function inflateZipEntry(compressionMethod: number, bytes: Uint8Array) {
  ...
  if (compressionMethod !== 8) {
    throw new Error(translate("lib.zip.unsupported_store_deflate_only.error", { defaultValue: "Unsupported zip archive: only STORE and DEFLATE entries are supported." }));
  }
  if (typeof DecompressionStream !== "function") {
    throw new Error(translate("lib.zip.browser_cannot_read_compressed_entries.error", { defaultValue: "Unsupported zip archive: this browser cannot read compressed zip entries." }));
  }
...
export async function readZipArchive(source: ArrayBuffer | Uint8Array): Promise<{
  ...
  while (offset + 4 <= bytes.length) {
    ...
    if (signature !== 0x04034b50) {
      throw new Error(translate("lib.zip.unsupported_local_file_header.error", { defaultValue: "Invalid zip archive: unsupported local file header." }));
    }

    if (offset + 30 > bytes.length) {
      throw new Error(translate("lib.zip.truncated_local_file_header.error", { defaultValue: "Invalid zip archive: truncated local file header." }));
    }
    ...
    if ((generalPurposeFlag & 0x0008) !== 0) {
      throw new Error(translate("lib.zip.data_descriptors_not_supported.error", { defaultValue: "Unsupported zip archive: data descriptors are not supported." }));
    }
    ...
    if (bodyEnd > bytes.length) {
      throw new Error(translate("lib.zip.truncated_file_contents.error", { defaultValue: "Invalid zip archive: truncated file contents." }));
    }
