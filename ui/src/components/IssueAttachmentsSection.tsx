...
const { t } = useTranslation();
  ...
  return (
    <div className="flex shrink-0 items-center gap-1">
      <Button asChild variant="ghost" size="icon-sm" title={t("components.issueattachmentssection.open_in_new_tab.attr_title", { defaultValue: "Open in new tab" })}>
        <a href={attachmentOpenPath(attachment)} target="_blank" rel="noreferrer" aria-label={`Open ${filename}`}>
          <ExternalLink className="h-4 w-4" />
        </a>
      </Button>
      <Button asChild variant="ghost" size="icon-sm" title={t("components.issueattachmentssection.download.attr_title", { defaultValue: "Download" })}>
      ...
      <Button
        variant="ghost"
        size="icon-sm"
        title={t("components.issueattachmentssection.delete_attachment.attr_title", { defaultValue: "Delete attachment" })}
...
const { t } = useTranslation();
  ...
  return (
    <p className="mt-0.5 text-[11px] text-muted-foreground">
      {t("components.issueattachmentssection.attachment.jsx-text", { defaultValue: "\n      Attachment · " })}{attachment.contentType} · {formatBytes(attachment.byteSize)}
    </p>
  );
}
  ...
  return (
    <div id={`attachment-${attachment.id}`} className="scroll-mt-20 rounded-lg border border-border p-3">
      ...
      <div className="mt-3 rounded-md hover:bg-accent/10">
        {isLoading ? (
          <p className="px-1 py-2 text-xs text-muted-foreground">{t("components.issueattachmentssection.loading_preview.jsx-text", { defaultValue: "Loading preview..." })}</p>
        ) : error ? (
          <p className="px-1 py-2 text-xs text-destructive">{t("components.issueattachmentssection.could_not_load_markdown_preview.jsx-text", { defaultValue: "Could not load markdown preview." })}</p>
        ) : (
          <FoldCurtain>
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-muted-foreground">{t("components.issueattachmentssection.attachments.jsx-text", { defaultValue: "Attachments" })}</h3>
          ...
          {imageAttachments.map((attachment) => (
            ...
            >
              ...
              {confirmDeleteId === attachment.id ? (
                ...
                >
                  <p className="text-xs font-medium text-white">{t("components.issueattachmentssection.delete.jsx-text", { defaultValue: "Delete?" })}</p>
                  <div className="flex gap-1.5">
                    <button
                      ...
                      disabled={deletePending}
                    >
                      {t("components.issueattachmentssection.yes.jsx-text", { defaultValue: "\n                      Yes\n                    " })}</button>
                    <button
                      type="button"
                    ...
                    >
                      {t("components.issueattachmentssection.no.jsx-text", { defaultValue: "\n                      No\n                    " })}</button>
                  </div>
                </div>
              ) : (
                <button
      ...
      {confirmDeleteId && !imageAttachments.some((attachment) => attachment.id === confirmDeleteId) ? (
        <div className="flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
          <p className="text-sm font-medium text-destructive">{t("components.issueattachmentssection.delete_this_attachment_this_cann.jsx-text", { defaultValue: "Delete this attachment? This cannot be undone." })}</p>
          <div className="flex shrink-0 items-center gap-2">
            <Button variant="ghost" size="sm" onClick={() => setConfirmDeleteId(null)} disabled={deletePending}>
              {t("components.issueattachmentssection.cancel.jsx-text", { defaultValue: "\n              Cancel\n            " })}</Button>
