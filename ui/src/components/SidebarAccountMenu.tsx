...
export function SidebarAccountMenu({
  ...
  const displayName = session?.user.name?.trim() || t("account.displayName", { defaultValue: "Board" });
  const secondaryLabel =
    session?.user.email?.trim() ||
    (deploymentMode === "authenticated"
      ? t("account.signedInAs", { defaultValue: "Signed in" })
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <div className="-mt-8 px-4 pb-4">
            ...
            <div className="mt-4 flex items-center justify-between rounded-xl border border-border/60 bg-muted/40 px-3 py-2">
              <span className="text-xs font-medium text-muted-foreground">
                {t("account.locale.label", { defaultValue: "Language" })}
              </span>
              <LocaleSwitcher />
            </div>
            ...
            <div className="mt-2 space-y-1">
              ...
              <MenuAction
                label={
                  theme === "dark"
                    ? t("account.switchToLightMode", { defaultValue: "Switch to light mode" })
                    : t("account.switchToDarkMode", { defaultValue: "Switch to dark mode" })
                }
                description={t("account.themeDescription", { defaultValue: "Toggle the app appearance." })}
