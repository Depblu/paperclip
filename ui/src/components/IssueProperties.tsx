    ...
    <>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("components.issueproperties.remove_blocker.jsx-text", { defaultValue: "Remove blocker?" })}</DialogTitle>
            <DialogDescription>
              {t("components.issueproperties.remove.jsx-text", { defaultValue: "\n              Remove " })}{confirmLabel} {t("components.issueproperties.as_a_blocker_for_this_task.jsx-text", { defaultValue: " as a blocker for this task.\n            " })}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
              <Button type="button" variant="outline">{t("components.issueproperties.cancel.jsx-text", { defaultValue: "Cancel" })}</Button>
            </DialogClose>
            <Button type="button" variant="destructive" onClick={confirmRemove}>
              {t("components.issueproperties.remove_blocker.jsx-text", { defaultValue: "\n              Remove blocker\n            " })}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
...
const { t } = useTranslation();
  ...
  const assigneeOptionsTrigger = (() => {
    if (assigneeOverrideLane === "cheap") {
      return <span className="text-sm">{t("components.issueproperties.cheap_model.jsx-text", { defaultValue: "Cheap model" })}</span>;
    }
    if (assigneeOverrideLane === "custom") {
      ...
      return (
        <span className="min-w-0 text-sm break-words">
          {t("components.issueproperties.custom.jsx-text", { defaultValue: "\n          Custom" })}{details.length > 0 ? ` · ${details.join(" · ")}` : " adapter options"}
        </span>
      );
    }
    return <span className="text-sm text-muted-foreground">{t("components.issueproperties.primary_model.jsx-text", { defaultValue: "Primary model" })}</span>;
  })();
  const assigneeOptionsContent = supportsAssigneeOverrides ? (
    <div className="w-full space-y-3 p-2">
      <div className="space-y-1.5">
        <div className="text-xs text-muted-foreground">{t("components.issueproperties.model_lane.jsx-text", { defaultValue: "Model lane" })}</div>
        <div className="flex w-full overflow-hidden rounded-md border border-border" role="radiogroup" aria-label={t("components.issueproperties.model_lane.attr_aria-label", { defaultValue: "Model lane" })}>
        ...
        {assigneeOverrideLane === "cheap" ? (
          <p className="text-[11px] text-muted-foreground">
            {t("components.issueproperties.sends.jsx-text", { defaultValue: "\n            Sends " })}<code>{t("components.issueproperties.modelprofile_cheap.jsx-text", { defaultValue: "modelProfile: \"cheap\"" })}</code>{" "}
            {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
              ? <>{t("components.issueproperties.adapter_default.jsx-text", { defaultValue: "· adapter default " })}<code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
              : assigneeCheapProfile
                ? <>{t("components.issueproperties.uses_the_agent_apos_s_configured.jsx-text", { defaultValue: "· uses the agent&apos;s configured cheap profile" })}</>
                : <>{t("components.issueproperties.falls_back_to_the_primary_model_.jsx-text", { defaultValue: "· falls back to the primary model if no cheap profile is configured" })}</>}
          </p>
        ) : null}
      </div>
      {assigneeOverrideLane === "custom" ? (
        <>
          <div className="space-y-1.5">
            <div className="text-xs text-muted-foreground">{t("components.issueproperties.model.jsx-text", { defaultValue: "Model" })}</div>
          ...
          <div className="space-y-1.5">
            <div className="text-xs text-muted-foreground">{t("components.issueproperties.thinking_effort.jsx-text", { defaultValue: "Thinking effort" })}</div>
          ...
          {assigneeAdapterType === "claude_local" ? (
            <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
              <div className="text-xs text-muted-foreground">{t("components.issueproperties.enable_chrome_chrome.jsx-text", { defaultValue: "Enable Chrome (--chrome)" })}</div>
              <ToggleSwitch
    ...
    <div className="w-full space-y-2 p-2">
      ...
      >
        {t("components.issueproperties.clear_adapter_options.jsx-text", { defaultValue: "\n        Clear adapter options\n      " })}</button>
    </div>
  );
  ...
  const reviewerTrigger = reviewerValues.length > 0
    ...
    : <span className="text-sm text-muted-foreground">{t("components.issueproperties.none.jsx-text", { defaultValue: "None" })}</span>;
  ...
  const monitorAttemptBadge = issue.monitorAttemptCount && issue.monitorAttemptCount > 0 ? (
    <span className="text-xs text-muted-foreground">
      {t("components.issueproperties.attempt.jsx-text", { defaultValue: "\n      Attempt " })}{issue.monitorAttemptCount}
    </span>
  ) : null;
  ...
  const scheduledRetryAttemptBadge = scheduledRetryAttempt !== null ? (
    <span className="text-xs text-muted-foreground">{t("components.issueproperties.attempt.jsx-text", { defaultValue: "Attempt " })}{scheduledRetryAttempt}</span>
  ) : null;
    ...
    <div className="flex w-full flex-col gap-2 p-2 text-xs">
      ...
      <dl className="grid grid-cols-[6rem_1fr] gap-y-1">
        {scheduledRetryReasonLabel ? (
          <>
            <dt className="text-muted-foreground">{t("components.issueproperties.reason.jsx-text", { defaultValue: "Reason" })}</dt>
            <dd className="text-foreground">{scheduledRetryReasonLabel}</dd>
          </>
        ) : null}
        {scheduledRetryAbsolute ? (
          <>
            <dt className="text-muted-foreground">{t("components.issueproperties.next_attempt.jsx-text", { defaultValue: "Next attempt" })}</dt>
        ...
        {scheduledRetry.retryOfRunId ? (
          <>
            <dt className="text-muted-foreground">{t("components.issueproperties.replaces_run.jsx-text", { defaultValue: "Replaces run" })}</dt>
            <dd className="text-foreground">
              <Link
        ...
        {scheduledRetry.agentName ? (
          <>
            <dt className="text-muted-foreground">{t("components.issueproperties.agent.jsx-text", { defaultValue: "Agent" })}</dt>
            <dd className="text-foreground">
              <Link
        ...
        {scheduledRetry.error ? (
          <>
            <dt className="text-muted-foreground">{t("components.issueproperties.last_error.jsx-text", { defaultValue: "Last error" })}</dt>
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("components.issueproperties.retrying.jsx-text", { defaultValue: "\n              Retrying…\n            " })}</span>
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("components.issueproperties.retry_now.jsx-text", { defaultValue: "\n              Retry now\n            " })}</span>
          )}
        </Button>
    ...
    <div className="flex w-full flex-col gap-2">
      <div className="flex flex-col gap-2 md:flex-row">
        ...
        <input
          ...
          placeholder={t("components.issueproperties.what_should_the_agent_re_check.attr_placeholder", { defaultValue: "What should the agent re-check?" })}
      ...
      <div className="flex flex-col gap-2 md:flex-row">
        <input
          type="text"
          className="min-w-0 flex-1 rounded-md border border-border bg-transparent px-2 py-1 text-xs"
          placeholder={t("components.issueproperties.external_service.attr_placeholder", { defaultValue: "External service" })}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={saveMonitor}
          >
            {t("components.issueproperties.schedule.jsx-text", { defaultValue: "\n            Schedule\n          " })}</button>
          {issue.executionPolicy?.monitor ? (
            ...
            >
              {t("components.issueproperties.clear.jsx-text", { defaultValue: "\n              Clear\n            " })}</button>
          ) : null}
        </div>
      </div>
    </div>
  ...
  const labelsTrigger = selectedIssueLabels.length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("components.issueproperties.no_labels.jsx-text", { defaultValue: "No labels" })}</span>
    </>
  );
  const labelsExtra = (issue.labelIds ?? []).length > 0 ? (
    <button
      ...
      aria-label={t("components.issueproperties.add_label.attr_aria-label", { defaultValue: "Add label" })}
      title={t("components.issueproperties.add_label.attr_title", { defaultValue: "Add label" })}
    >
  ...
  const labelsContent = (
    <>
      <input
        ...
        placeholder={t("components.issueproperties.search_labels.attr_placeholder", { defaultValue: "Search labels..." })}
        value={labelSearch}
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("components.issueproperties.new_label.attr_placeholder", { defaultValue: "New label" })}
            value={newLabelName}
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">{t("components.issueproperties.unassigned.jsx-text", { defaultValue: "Unassigned" })}</span>
    </>
  );
  ...
  const assigneeContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("components.issueproperties.search_assignees.attr_placeholder", { defaultValue: "Search assignees..." })}
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {t("components.issueproperties.assign_to_me.jsx-text", { defaultValue: "\n            Assign to me\n          " })}</button>
        )}
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          <button
            className={cn(
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("components.issueproperties.no_project.jsx-text", { defaultValue: "No project" })}</span>
    </>
  );
  ...
  const projectContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("components.issueproperties.search_projects.attr_placeholder", { defaultValue: "Search projects..." })}
        value={projectSearch}
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">{t("components.issueproperties.no_parent.jsx-text", { defaultValue: "No parent" })}</span>
  );
  ...
  const parentContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("components.issueproperties.search_tasks.attr_placeholder", { defaultValue: "Search tasks..." })}
        value={parentSearch}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {t("components.issueproperties.no_parent.jsx-text", { defaultValue: "\n          No parent\n        " })}</button>
  ...
  const blockedByContent = (
    <>
      <input
        ...
        autoFocus={!inline}
        aria-label={t("components.issueproperties.search_tasks_to_add_as_blockers.attr_aria-label", { defaultValue: "Search tasks to add as blockers" })}
      />
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          className={cn(
        ...
        >
          {t("components.issueproperties.no_blockers.jsx-text", { defaultValue: "\n          No blockers\n        " })}</button>
        ...
        {blockerOptionsLoading ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("components.issueproperties.searching_tasks.jsx-text", { defaultValue: "Searching tasks..." })}</div>
        ) : blockerOptions.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("components.issueproperties.no_matching_tasks.jsx-text", { defaultValue: "No matching tasks." })}</div>
        ) : null}
      </div>
    </>
  );
  const renderAddBlockedByButton = (onClick?: () => void) => (
    ...
    >
      <Plus className="h-3 w-3" />
      {t("components.issueproperties.add_blocker.jsx-text", { defaultValue: "\n      Add blocker\n    " })}</button>
  );

  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("components.issueproperties.status.attr_label", { defaultValue: "Status" })}>
          <StatusIcon
            status={issue.status}
        ...
        <PropertyRow label={t("components.issueproperties.priority.attr_label", { defaultValue: "Priority" })}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.labels.attr_label", { defaultValue: "Labels" })}
          open={labelsOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.assignee.attr_label", { defaultValue: "Assignee" })}
          open={assigneeOpen}
        ...
        {showAssigneeAdapterOptions ? (
          <PropertyPicker
            inline={inline}
            label={t("components.issueproperties.model.attr_label", { defaultValue: "Model" })}
            ...
            extra={
              <button
                ...
                className="inline-flex items-center justify-center h-5 w-5 rounded hover:bg-accent/50 transition-colors text-muted-foreground hover:text-foreground"
                onClick={() => updateAssigneeAdapterOverrides(null)}
                aria-label={t("components.issueproperties.clear_adapter_options.attr_aria-label", { defaultValue: "Clear adapter options" })}
                title={t("components.issueproperties.clear_adapter_options.attr_title", { defaultValue: "Clear adapter options" })}
              >
                <X className="h-3 w-3" />
              </button>
            }
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.project.attr_label", { defaultValue: "Project" })}
          open={projectOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.parent.attr_label", { defaultValue: "Parent" })}
          open={parentOpen}
          onOpenChange={(open) => {
            setParentOpen(open);
        ...
        {inline ? (
          <div>
            <PropertyRow label={t("components.issueproperties.blocked_by.attr_label", { defaultValue: "Blocked by" })}>
        ...
        <PropertyRow label={t("components.issueproperties.blocking.attr_label", { defaultValue: "Blocking" })}>
        ...
        <PropertyRow label={t("components.issueproperties.sub_tasks.attr_label", { defaultValue: "Sub-tasks" })}>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {onAddSubIssue ? (
              ...
              >
                <Plus className="h-3 w-3" />
              {t("components.issueproperties.add_sub_task.jsx-text", { defaultValue: "\n              Add sub-task\n              " })}</button>
            ) : null}
          </div>
        </PropertyRow>

        {relatedTasks.length > 0 ? (
          <PropertyRow label={t("components.issueproperties.related_tasks.attr_label", { defaultValue: "Related Tasks" })}>
            <div className="flex flex-wrap gap-1">
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.reviewers.attr_label", { defaultValue: "Reviewers" })}
          open={reviewersOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.approvers.attr_label", { defaultValue: "Approvers" })}
          open={approversOpen}
        ...
        {currentExecutionLabel && (
          <PropertyRow label={t("components.issueproperties.execution.attr_label", { defaultValue: "Execution" })}>
        ...
        {showScheduledRetryRow && scheduledRetryContent ? (
          <PropertyPicker
            inline={inline}
            label={t("components.issueproperties.scheduled_retry.attr_label", { defaultValue: "Scheduled retry" })}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueproperties.monitor.attr_label", { defaultValue: "Monitor" })}
          open={monitorOpen}
        ...
        {issue.requestDepth > 0 && (
          <PropertyRow label={t("components.issueproperties.depth.attr_label", { defaultValue: "Depth" })}>
        ...
        <>
          ...
          <div className="space-y-1">
            {liveWorkspaceService?.url && (
              <PropertyRow label={t("components.issueproperties.service.attr_label", { defaultValue: "Service" })}>
                <a
            ...
            {showWorkspaceDetailLink && issue.executionWorkspaceId && (
              <PropertyRow label={t("components.issueproperties.workspace.attr_label", { defaultValue: "Workspace" })}>
                <Link
                  to={`/execution-workspaces/${issue.executionWorkspaceId}`}
                  className="text-sm text-primary hover:underline inline-flex items-center gap-1"
                >
                  {t("components.issueproperties.view_workspace.jsx-text", { defaultValue: "\n                  View workspace\n                  " })}<ExternalLink className="h-3 w-3" />
                </Link>
              </PropertyRow>
            )}
            {issue.currentExecutionWorkspace?.branchName && (
              <PropertyRow label={t("components.issueproperties.branch.attr_label", { defaultValue: "Branch" })}>
                <TruncatedCopyable
            ...
            {issue.currentExecutionWorkspace?.cwd && (
              <PropertyRow label={t("components.issueproperties.folder.attr_label", { defaultValue: "Folder" })}>
                <TruncatedCopyable
      ...
      <div className="space-y-1">
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label={t("components.issueproperties.created_by.attr_label", { defaultValue: "Created by" })}>
        ...
        {issue.startedAt && (
          <PropertyRow label={t("components.issueproperties.started.attr_label", { defaultValue: "Started" })}>
            <span className="text-sm">{formatDateTime(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label={t("components.issueproperties.completed.attr_label", { defaultValue: "Completed" })}>
            <span className="text-sm">{formatDateTime(issue.completedAt)}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("components.issueproperties.created.attr_label", { defaultValue: "Created" })}>
          <span className="text-sm">{formatDateTime(issue.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("components.issueproperties.updated.attr_label", { defaultValue: "Updated" })}>
