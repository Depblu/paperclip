  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("components.cloudaccessgate.no_company_access.jsx-text", { defaultValue: "No company access" })}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("components.cloudaccessgate.this_account_is_signed_in_but_it.jsx-text", { defaultValue: "\n          This account is signed in, but it does not have an active company membership or instance-admin access on this Paperclip instance.\n        " })}</p>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("components.cloudaccessgate.use_a_company_invite_or_sign_in_.jsx-text", { defaultValue: "\n          Use a company invite or sign in with an account that already belongs to this org.\n        " })}</p>
      </div>
    </div>
  );
}
...
const { t } = useTranslation();
  ...
  if (
    ...
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("components.cloudaccessgate.loading.jsx-text", { defaultValue: "Loading..." })}</div>;
  }
