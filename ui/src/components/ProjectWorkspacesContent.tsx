...
const { t } = useTranslation();
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("components.projectworkspacescontent.no_non_default_workspace_activit.jsx-text", { defaultValue: "No non-default workspace activity yet." })}</p>;
  }
        ...
        {cleanupFailedSummaries.length > 0 ? (
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("components.projectworkspacescontent.cleanup_attention_needed.jsx-text", { defaultValue: "\n              Cleanup attention needed\n            " })}</div>
            <div className="space-y-3">
