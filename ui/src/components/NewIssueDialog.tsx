...
const { t } = useTranslation();
  ...
  return (
    <textarea
      ...
      placeholder={t("components.newissuedialog.task_title.attr_placeholder", { defaultValue: "Task title" })}
      rows={1}
      value={draftValue}
      onChange={(e) => {
...
const { t } = useTranslation();
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={t("components.newissuedialog.add_description.attr_placeholder", { defaultValue: "Add description..." })}
      bordered={false}
      mentions={mentions}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              disabled={createIssue.isPending}
            >
              <span className="text-lg leading-none">{t("components.newissuedialog.times.jsx-text", { defaultValue: "&times;" })}</span>
            </Button>
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("components.newissuedialog.for.jsx-text", { defaultValue: "For" })}</span>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("components.newissuedialog.assignee.jsx-text", { defaultValue: "Assignee" })}</span>
                  )
                }
                renderOption={(option) => {
                  ...
                  return (
                    <>
                      ...
                      {assignee && getTrustPreset(assignee.permissions) === "low_trust_review" ? (
                        <ShieldAlert className="ml-auto h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" aria-label={t("components.newissuedialog.low_trust_review_agent.attr_aria-label", { defaultValue: "Low-trust review agent" })} />
                      ) : null}
                    </>
                  );
                }}
              />
              <span>{t("components.newissuedialog.in.jsx-text", { defaultValue: "in" })}</span>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("components.newissuedialog.project.jsx-text", { defaultValue: "Project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={t("components.newissuedialog.add_reviewer_or_approver.attr_title", { defaultValue: "Add reviewer or approver" })}
                  >
                ...
                <PopoverContent className="w-44 p-1" align="start">
                  ...
                  >
                    ...
                    {t("components.newissuedialog.reviewer.jsx-text", { defaultValue: "\n                    Reviewer\n                  " })}</button>
                  <button
                    className={cn(
                      "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
                  ...
                  >
                    ...
                    {t("components.newissuedialog.approver.jsx-text", { defaultValue: "\n                    Approver\n                  " })}</button>
                </PopoverContent>
              </Popover>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("components.newissuedialog.reviewer.jsx-text", { defaultValue: "Reviewer" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("components.newissuedialog.approver.jsx-text", { defaultValue: "Approver" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("components.newissuedialog.sub_task_of.jsx-text", { defaultValue: "Sub-task of" })}</span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("components.newissuedialog.execution_workspace.jsx-text", { defaultValue: "Execution workspace" })}</div>
              <div className="text-[11px] text-muted-foreground">
                {t("components.newissuedialog.control_whether_this_task_runs_i.jsx-text", { defaultValue: "\n                Control whether this task runs in the shared workspace, a new isolated workspace, or an existing one.\n              " })}</div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && (
                ...
                >
                  <option value="">{t("components.newissuedialog.choose_an_existing_workspace.jsx-text", { defaultValue: "Choose an existing workspace" })}</option>
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("components.newissuedialog.reusing.jsx-text", { defaultValue: "\n                  Reusing " })}{selectedReusableExecutionWorkspace.name} {t("components.newissuedialog.from.jsx-text", { defaultValue: " from " })}{selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? "existing execution workspace"}.
                </div>
              )}
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("components.newissuedialog.model_lane.jsx-text", { defaultValue: "Model lane" })}</div>
                  <div
                    className="flex w-full overflow-hidden rounded-md border border-border"
                    role="radiogroup"
                    aria-label={t("components.newissuedialog.model_lane.attr_aria-label", { defaultValue: "Model lane" })}
                  >
                  ...
                  {assigneeModelLane === "cheap" && (
                    <p className="text-[11px] text-muted-foreground">
                      {t("components.newissuedialog.sends.jsx-text", { defaultValue: "\n                      Sends " })}<code>{t("components.newissuedialog.modelprofile_cheap.jsx-text", { defaultValue: "modelProfile: \"cheap\"" })}</code>{" "}
                      {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
                        ? <>{t("components.newissuedialog.adapter_default.jsx-text", { defaultValue: "· adapter default " })}<code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
                        : assigneeCheapProfile
                          ? <>{t("components.newissuedialog.uses_the_agent_s_configured_chea.jsx-text", { defaultValue: "· uses the agent's configured cheap profile" })}</>
                          : <>{t("components.newissuedialog.falls_back_to_the_primary_model_.jsx-text", { defaultValue: "· falls back to the primary model if no cheap profile is configured" })}</>}
                    </p>
                  )}
                  {assigneeModelLane === "primary" && (
                    <p className="text-[11px] text-muted-foreground">{t("components.newissuedialog.runs_on_the_agent_s_primary_mode.jsx-text", { defaultValue: "Runs on the agent's primary model." })}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-[11px] text-muted-foreground">{t("components.newissuedialog.override_the_model_and_effort_fo.jsx-text", { defaultValue: "Override the model and effort for this task only." })}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("components.newissuedialog.model.jsx-text", { defaultValue: "Model" })}</div>
                    <InlineEntitySelector
                ...
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("components.newissuedialog.thinking_effort.jsx-text", { defaultValue: "Thinking effort" })}</div>
                ...
                {assigneeAdapterType === "claude_local" && assigneeModelLane === "custom" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("components.newissuedialog.enable_chrome_chrome.jsx-text", { defaultValue: "Enable Chrome (--chrome)" })}</div>
                    <ToggleSwitch
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("components.newissuedialog.documents.jsx-text", { defaultValue: "Documents" })}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("components.newissuedialog.remove_document.attr_title", { defaultValue: "Remove document" })}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("components.newissuedialog.attachments.jsx-text", { defaultValue: "Attachments" })}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("components.newissuedialog.remove_attachment.attr_title", { defaultValue: "Remove attachment" })}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              >
                {currentPriority ? (
                  ...
                  <>
                    ...
                    {t("components.newissuedialog.priority.jsx-text", { defaultValue: "\n                    Priority\n                  " })}</>
                )}
              </button>
            </PopoverTrigger>
          ...
          >
            ...
            {t("components.newissuedialog.upload.jsx-text", { defaultValue: "\n            Upload\n          " })}</button>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("components.newissuedialog.start_date.jsx-text", { defaultValue: "\n                Start date\n              " })}</button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("components.newissuedialog.due_date.jsx-text", { defaultValue: "\n                Due date\n              " })}</button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("components.newissuedialog.assigning_implies_executable_int.jsx-text", { defaultValue: "\n              Assigning implies executable intent — leave status as " })}<span className="font-medium">{t("components.newissuedialog.backlog.jsx-text", { defaultValue: "Backlog" })}</span> {t("components.newissuedialog.only_to_deliberately_park_this_t.jsx-text", { defaultValue: " only to deliberately park this. The assignee will not be woken until status moves to " })}<span className="font-medium">{t("components.newissuedialog.todo.jsx-text", { defaultValue: "Todo" })}</span> {t("components.newissuedialog.or.jsx-text", { defaultValue: " or " })}<span className="font-medium">{t("components.newissuedialog.in_progress.jsx-text", { defaultValue: "In Progress" })}</span>.
            </span>
          </div>
        ) : null}
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          >
            {t("components.newissuedialog.discard_draft.jsx-text", { defaultValue: "\n            Discard Draft\n          " })}</Button>
          <div className="flex items-center gap-3">
            <div className="min-h-5 text-right">
              {createIssue.isPending ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                  <Loader2 className="h-3 w-3 animate-spin" />
                  {t("components.newissuedialog.creating_issue.jsx-text", { defaultValue: "\n                  Creating issue...\n                " })}</span>
              ) : createIssue.isError ? (
