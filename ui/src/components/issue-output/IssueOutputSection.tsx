...
const { t } = useTranslation();
  ...
  return (
    <section className="space-y-3" aria-label={t("components.issueoutputsection.task_outputs.attr_aria-label", { defaultValue: "Task outputs" })}>
      <div className="flex items-center gap-2">
        <Play className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        <h3 className="text-sm font-medium text-muted-foreground">{t("components.issueoutputsection.output.jsx-text", { defaultValue: "Output" })}</h3>
      ...
      {rest.length > 0 && (
        <div className="space-y-2">
          <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("components.issueoutputsection.also_produced.jsx-text", { defaultValue: "Also produced" })}</p>
          {rest.map((item) => (
