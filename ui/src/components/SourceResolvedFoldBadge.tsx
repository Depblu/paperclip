...
const { t } = useTranslation();
  ...
  return (
    <span
      ...
      title={title}
      aria-label={t("components.sourceresolvedfoldbadge.source_resolved_watchdog_fold.attr_aria-label", { defaultValue: "Source-resolved watchdog fold" })}
    >
      {showIcon ? <Sparkles className="h-3 w-3 text-emerald-700 dark:text-emerald-300" aria-hidden /> : null}
      {t("components.sourceresolvedfoldbadge.source_resolved.jsx-text", { defaultValue: "\n      Source-resolved\n    " })}</span>
  );
}
