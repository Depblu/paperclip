    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("components.agentconfigform.unsaved_changes.jsx-text", { defaultValue: "Unsaved changes" })}</span>
            <Button
              size="sm"
              onClick={handleSave}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentconfigform.identity.jsx-text", { defaultValue: "Identity" })}</h3>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("components.agentconfigform.name.attr_label", { defaultValue: "Name" })} hint={help.name}>
              <DraftInput
                ...
                placeholder={t("components.agentconfigform.agent_name.attr_placeholder", { defaultValue: "Agent name" })}
              />
            </Field>
            <Field label={t("components.agentconfigform.title.attr_label", { defaultValue: "Title" })} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("components.agentconfigform.e_g_vp_of_engineering.attr_placeholder", { defaultValue: "e.g. VP of Engineering" })}
              />
            </Field>
            <Field label={t("components.agentconfigform.reports_to.attr_label", { defaultValue: "Reports to" })} hint={help.reportsTo}>
              <ReportsToPicker
            ...
            <Field label={t("components.agentconfigform.capabilities.attr_label", { defaultValue: "Capabilities" })} hint={help.capabilities}>
              <MarkdownEditor
                ...
                placeholder={t("components.agentconfigform.describe_what_this_agent_can_do.attr_placeholder", { defaultValue: "Describe what this agent can do..." })}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("components.agentconfigform.prompt_template.attr_label", { defaultValue: "Prompt Template" })} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("components.agentconfigform.you_are_agent_agent_name_your_ro.attr_placeholder", { defaultValue: "You are agent {{ agent.name }}. Your role is {{ agent.role }}..." })}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("components.agentconfigform.prompt_template_is_replayed_on_e.jsx-text", { defaultValue: "\n                  Prompt template is replayed on every heartbeat. Keep it compact and dynamic to avoid recurring token cost and cache churn.\n                " })}</div>
              </>
            )}
          </div>
        </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentconfigform.execution.jsx-text", { defaultValue: "Execution" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("components.agentconfigform.execution.jsx-text", { defaultValue: "Execution" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
            ...
            >
              ...
              >
                <option value="">{t("components.agentconfigform.company_default_local.jsx-text", { defaultValue: "Company default (Local)" })}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("components.agentconfigform.adapter.jsx-text", { defaultValue: "Adapter" })}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("components.agentconfigform.adapter.jsx-text", { defaultValue: "Adapter" })}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("components.agentconfigform.adapter_type.attr_label", { defaultValue: "Adapter type" })} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("components.agentconfigform.working_directory_deprecated.attr_label", { defaultValue: "Working directory (deprecated)" })} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentconfigform.permissions_amp_configuration.jsx-text", { defaultValue: "Permissions &amp; Configuration" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("components.agentconfigform.permissions_amp_configuration.jsx-text", { defaultValue: "Permissions &amp; Configuration" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("components.agentconfigform.command.attr_label", { defaultValue: "Command" })} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("components.agentconfigform.primary_model.jsx-text", { defaultValue: "Primary model" })}</div>
              )}
              <ModelDropdown
                models={models}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("components.agentconfigform.codex_may_reject_minimal_thinkin.jsx-text", { defaultValue: "\n                        Codex may reject `minimal` thinking when search is enabled.\n                      " })}</p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("components.agentconfigform.bootstrap_prompt_legacy.attr_label", { defaultValue: "Bootstrap prompt (legacy)" })} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("components.agentconfigform.optional_initial_setup_prompt_fo.attr_placeholder", { defaultValue: "Optional initial setup prompt for the first run" })}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
              ...
              <Field label={t("components.agentconfigform.extra_args_comma_separated.attr_label", { defaultValue: "Extra args (comma-separated)" })} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("components.agentconfigform.e_g_verbose_foo_bar.attr_placeholder", { defaultValue: "e.g. --verbose, --foo=bar" })}
                />
              </Field>

              <Field label={t("components.agentconfigform.environment_variables.attr_label", { defaultValue: "Environment variables" })} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("components.agentconfigform.timeout_sec.attr_label", { defaultValue: "Timeout (sec)" })} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("components.agentconfigform.interrupt_grace_period_sec.attr_label", { defaultValue: "Interrupt grace period (sec)" })} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("components.agentconfigform.run_policy.jsx-text", { defaultValue: " Run Policy" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("components.agentconfigform.run_policy.jsx-text", { defaultValue: " Run Policy" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("components.agentconfigform.heartbeat_on_interval.attr_label", { defaultValue: "Heartbeat on interval" })}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("components.agentconfigform.advanced_run_policy.attr_title", { defaultValue: "Advanced Run Policy" })}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("components.agentconfigform.wake_on_demand.attr_label", { defaultValue: "Wake on demand" })}
              ...
              />
              <Field label={t("components.agentconfigform.cooldown_sec.attr_label", { defaultValue: "Cooldown (sec)" })} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("components.agentconfigform.max_concurrent_runs.attr_label", { defaultValue: "Max concurrent runs" })} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("components.agentconfigform.continue_after_max_turn_stop.attr_label", { defaultValue: "Continue after max-turn stop" })}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("components.agentconfigform.continuation_attempts.attr_label", { defaultValue: "Continuation attempts" })} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("components.agentconfigform.continuation_delay_sec.attr_label", { defaultValue: "Continuation delay (sec)" })} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("components.agentconfigform.hint.jsx-text", { defaultValue: "Hint: " })}{check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("components.agentconfigform.coming_soon.jsx-text", { defaultValue: "Coming soon" })}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
const { t } = useTranslation();
  ...
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium leading-none text-amber-700 dark:text-amber-200">
      {t("components.agentconfigform.experimental.jsx-text", { defaultValue: "\n      Experimental\n    " })}</span>
  );
}
...
const { t } = useTranslation();
  ...
  return (
    <Field label={t("components.agentconfigform.model.attr_label", { defaultValue: "Model" })} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {t("components.agentconfigform.current.jsx-text", { defaultValue: "\n                current\n              " })}</span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {t("components.agentconfigform.detected.jsx-text", { defaultValue: "\n                detected\n              " })}</span>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">
                    {t("components.agentconfigform.config.jsx-text", { defaultValue: "\n                    config\n                  " })}</span>
                </button>
              );
            })}
          <div className="max-h-[240px] overflow-y-auto">
            {allowDefault && (
              ...
              >
                {t("components.agentconfigform.default.jsx-text", { defaultValue: "\n                Default\n              " })}</button>
            )}
            {canCreateManualModel && (
              ...
              >
                <span>{t("components.agentconfigform.use_manual_model.jsx-text", { defaultValue: "Use manual model" })}</span>
    ...
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("components.agentconfigform.cheap_model.jsx-text", { defaultValue: "Cheap model" })}</div>
          <p className="text-xs text-muted-foreground">
            {t("components.agentconfigform.used_when_a_run_requests_the_che.jsx-text", { defaultValue: "\n            Used when a run requests the cheap profile (e.g. routine summaries). The primary model stays unchanged.\n          " })}</p>
        </div>
...
const { t } = useTranslation();
  ...
  return (
    <Field label={t("components.agentconfigform.thinking_effort.attr_label", { defaultValue: "Thinking effort" })} hint={help.thinkingEffort}>
