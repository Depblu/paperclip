...
const { t } = useTranslation();
  ...
  if (!resolvedCompanyId) {
    ...
    return <div className="text-sm text-muted-foreground">{t("pages.companysettingspluginpage.select_a_company_to_view_this_pa.jsx-text", { defaultValue: "Select a company to view this page." })}</div>;
  }

  if (!settingsRoutePath || isLoading) {
    return <div className="text-sm text-muted-foreground">{t("pages.companysettingspluginpage.loading.jsx-text", { defaultValue: "Loading..." })}</div>;
  }

  if (errorMessage) {
    return (
      <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
        {t("pages.companysettingspluginpage.plugin_extensions_unavailable.jsx-text", { defaultValue: "\n        Plugin extensions unavailable: " })}{errorMessage}
      </div>
    );
  }
  ...
  if (pageSlots.length > 1) {
    return (
      <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
        {t("pages.companysettingspluginpage.multiple_plugins_declare_the_com.jsx-text", { defaultValue: "\n        Multiple plugins declare the company settings route " })}<code>{settingsRoutePath}</code>{t("pages.companysettingspluginpage.disable_one_plugin_or_change_its.jsx-text", { defaultValue: ". Disable one plugin or change its route.\n      " })}</div>
    );
  }
