    ...
    <div data-testid="issue-chat-long-thread-perf" className="space-y-5">
      <div className="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {t("pages.issuechatlongthreadperf.deterministic_local_fixture_for_.jsx-text", { defaultValue: "\n            Deterministic local fixture for measuring the current direct-render issue chat path with hundreds of merged thread rows, markdown-heavy assistant bodies, linked runs, documents, sub-issues, and sidebar context.\n          " })}</p>
        </div>
        <div className="grid min-w-[280px] grid-cols-2 gap-2">
          <MetricTile label={t("pages.issuechatlongthreadperf.fixture_rows.attr_label", { defaultValue: "Fixture rows" })} value={String(rowTarget)} testId="perf-fixture-row-target" />
          <MetricTile label={t("pages.issuechatlongthreadperf.markdown_rows.attr_label", { defaultValue: "Markdown rows" })} value={String(LONG_THREAD_MARKDOWN_COMMENT_COUNT)} testId="perf-fixture-markdown-rows" />
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <main className="min-w-0 space-y-4">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("pages.issuechatlongthreadperf.issue_documents.jsx-text", { defaultValue: "Issue documents" })}</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("pages.issuechatlongthreadperf.sub_issues.jsx-text", { defaultValue: "Sub-issues" })}</CardTitle>
            </CardHeader>
        ...
        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("pages.issuechatlongthreadperf.baseline_metrics.jsx-text", { defaultValue: "Baseline metrics" })}</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2">
              <MetricTile label={t("pages.issuechatlongthreadperf.profiler_commits.attr_label", { defaultValue: "Profiler commits" })} value={String(metrics.commitCount)} testId="perf-commit-count" />
              <MetricTile label={t("pages.issuechatlongthreadperf.mount_duration.attr_label", { defaultValue: "Mount duration" })} value={formatMs(metrics.mountActualDuration)} testId="perf-mount-duration" />
              <MetricTile label={t("pages.issuechatlongthreadperf.latest_duration.attr_label", { defaultValue: "Latest duration" })} value={formatMs(metrics.latestActualDuration)} testId="perf-latest-duration" />
              <MetricTile label={t("pages.issuechatlongthreadperf.max_duration.attr_label", { defaultValue: "Max duration" })} value={formatMs(metrics.maxActualDuration)} testId="perf-max-duration" />
              <MetricTile label={t("pages.issuechatlongthreadperf.total_duration.attr_label", { defaultValue: "Total duration" })} value={formatMs(metrics.totalActualDuration)} testId="perf-total-duration" />
            </CardContent>
          </Card>

          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("pages.issuechatlongthreadperf.fixture_shape.jsx-text", { defaultValue: "Fixture shape" })}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
