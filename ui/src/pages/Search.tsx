  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-page="search">
      <div className="border-b border-border px-4 py-3 sm:px-6">
        <h1 className="sr-only">{t("pages.search.search.jsx-text", { defaultValue: "Search" })}</h1>
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t("pages.search.search_tasks_comments_documents_.attr_placeholder", { defaultValue: "Search tasks, comments, documents, artifacts, agents, projects…" })}
            aria-label={t("pages.search.search_query.attr_aria-label", { defaultValue: "Search query" })}
          ...
          {draftQuery.length > 0 ? (
            <button
              type="button"
              onClick={handleClear}
              aria-label={t("pages.search.clear_search.attr_aria-label", { defaultValue: "Clear search" })}
          ...
          >
            {t("pages.search.k.jsx-text", { defaultValue: "\n            ⌘K\n          " })}</kbd>
        </div>
      </div>
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("pages.search.type_to_search_company_memory.jsx-text", { defaultValue: "Type to search company memory." })}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("pages.search.tasks_comments_plan_documents_ar.jsx-text", { defaultValue: "\n            Tasks, comments, plan documents, artifacts, agents, projects — same surface, ranked by relevance.\n          " })}</p>
        </div>
        {recentSearches.length > 0 ? (
          <div>
            <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              {t("pages.search.recent_searches.jsx-text", { defaultValue: "\n              Recent searches\n            " })}</div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          ...
          <li>
            <span className="font-medium text-foreground">{t("pages.search.k.jsx-text", { defaultValue: "⌘K:" })}</span> {t("pages.search.reopens_the_command_palette_pre_.jsx-text", { defaultValue: " reopens the command palette pre-seeded with your current query.\n          " })}</li>
        </ul>
      </div>
    );
  }
      ...
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("pages.search.open_tasks_filter_view.jsx-text", { defaultValue: "\n            Open Tasks filter view\n          " })}</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("pages.search.searching_for_ldquo.jsx-text", { defaultValue: "\n          Searching for &ldquo;" })}{trimmedQuery}{t("pages.search.rdquo.jsx-text", { defaultValue: "&rdquo;…\n        " })}</div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("pages.search.search_all_scopes.jsx-text", { defaultValue: "\n              Search all scopes\n            " })}</Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("pages.search.create_task_from_this_query.jsx-text", { defaultValue: "\n            Create task from this query\n          " })}</Button>
          <Button onClick={navigateIssuesFallback} size="sm" variant="ghost">
            {t("pages.search.open_tasks_filter_view.jsx-text", { defaultValue: "\n            Open Tasks filter view\n          " })}</Button>
        </div>
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("pages.search.try_fewer_tokens_or_a_single_dis.jsx-text", { defaultValue: "Try fewer tokens or a single distinctive term." })}</li>
          <li>
            {t("pages.search.use_an_identifier_shortcut_like.jsx-text", { defaultValue: "\n            Use an identifier shortcut like " })}<code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("pages.search.wrap_multi_word_phrases_in_quote.jsx-text", { defaultValue: "Wrap multi-word phrases in quotes." })}</li>
        </ul>
      </div>
    );
  }
