...
- **Default locale**: `zh-CN` (also the fallback language).
...
The English file is the single source of truth. `zh-CN.json` must mirror
its structure exactly; `locale-validation.ts` enforces that on every test run.
...
3. If the value contains an interpolation placeholder (e.g. `{{name}}`),
   the same placeholder must appear in every locale file in the same
   position. The validator rejects drift.

4. Run `pnpm --filter @paperclipai/ui exec vitest run src/i18n/` to
   confirm the locale files still pass validation.

## Adding a new locale

1. Create `ui/src/i18n/locales/<bcp47>.json` (e.g. `en-AU.json`,
   `fr-CA.json`) with the same structure as `en.json`.
